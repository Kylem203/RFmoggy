...
void handlereplayMenu(){
  Serial.println("User called the Replay menu");
  server.send(200, "text/html", replay_menu);
}
...
void handlereplaySend() {
  String rawData = server.arg("rawData"); // Extract rawData from the request
  float frequency = server.arg("frequency").toFloat(); // Extract frequency from the request
  float dataRate = server.arg("datarate").toFloat(); // Extract data rate from the request

  if (rawData.length() == 0) {
    server.send(400, "text/html", "Bad Request: Missing rawData");
    return;
  }

  if (frequency <= 0 || dataRate <= 0) {
    server.send(400, "text/html", "Bad Request: Invalid frequency or dataRate");
    return;
  }
  ...
  Serial.println("TX: Raw Data");
  digitalWrite(LED_BUILTIN, LOW);

  pinMode(gdo0, OUTPUT);
  ELECHOUSE_cc1101.setMHZ(frequency);
  ELECHOUSE_cc1101.setModulation(2); // OOK modulation
  ELECHOUSE_cc1101.setDRate(1.0); // Set data rate
  ...
  while (end != -1) {
    int duration = rawData.substring(start, end).toInt();
    ...
    if (duration > 0) {
      ...
      duration = -duration; // Make the duration positive for delay
    }
  ...
  Serial.println("EOTX: Raw Data");

  server.send(200, "text/html", menu_htmlHeader + "<h1>Signal is sent</h1><p><a href=\"/\">Return to Main Menu</a></p><p><a href=\"replay\">Return to Replay Menu</a></p>");
}
...
/*************************************************************************/
  ...
  server.on("/replay", handlereplayMenu);
  ...
  server.on("/sendreplay", handlereplaySend);
  ...
  main_menu += "<p><a href=\"replay\">Replay Raw Menu</a></p>";
...
replay_menu += menu_htmlHeader;
replay_menu += "<title>Replay .sub ghz Raw data</title>";
replay_menu += "<body><center><h1>Replay Raw Timings data</h1>";
replay_menu += "<h3>Imbed your own Raw code</h3>";
replay_menu += "<form action=\"sendreplay\">";
replay_menu += "<label for=\"frequency\">Frequency (MHz):</label>";
replay_menu += "<input type=\"text\" id=\"frequency\" name=\"frequency\" value=\"315.0\"><br><br>";
replay_menu += "<label for=\"datarate\">Data Rate (kbps):</label>";
replay_menu += "<input type=\"text\" id=\"datarate\" name=\"datarate\" value=\"650\"><br><br>";
replay_menu += "<textarea name=\"rawData\" rows=\"10\" cols=\"50\"></textarea><br><br>";
replay_menu += "<button name=\"remotebutton\" type=\"submit\" value=\"submit\">Replay Raw</button></a><br><br>";
replay_menu += "</form><p><a href=\"/\">Return to Main Menu</a></p></body></html>";
